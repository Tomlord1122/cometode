    ...
    <div class="flex gap-2">
      ...
      <div class="relative">
        <button
          ...
          class="p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors {hasActiveFilters
            ? 'text-blue-500'
        ...
        {#if showFilterMenu}
              ...
              <div class="flex gap-1">
                {#each ['Easy', 'Medium', 'Hard'] as diff (diff)}
                  <button
                    ...
                    class="flex-1 px-2 py-1 text-xs rounded transition-colors {selectedDifficulties.includes(
                      diff
                    )
                      ? 'bg-indigo-400/90 text-white shadow-sm'
          ...
          <div
            ...
            onkeydown={(e) => {
              if (e.key === 'Escape') showFilterMenu = false
            }}
          ></div>
        {/if}
      </div>
